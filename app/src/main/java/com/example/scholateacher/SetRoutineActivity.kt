...
import com.example.scholateacher.databinding.ActivitySetRoutineBinding
import com.google.firebase.database.DataSnapshot
import com.google.firebase.database.DatabaseError
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.database.ValueEventListener

class SetRoutineActivity : AppCompatActivity() {
    lateinit var binding: ActivitySetRoutineBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivitySetRoutineBinding.inflate(layoutInflater)
    ...
    private fun setSpinners() {
        MyClass().getCurrentSection { section ->
            if (section != null) {
                ...
                assignCourseRef.orderByChild("section_id").equalTo(sectionId).addListenerForSingleValueEvent(object :
                    ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        if (snapshot.exists()) {
                            ...
                            spinners.forEach { spinner ->
                                val adapter = ArrayAdapter(applicationContext, android.R.layout.simple_spinner_item, courses)
                                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                                spinner.adapter = adapter
                                spinner.setSelection(-1)
                            }
                            roomSpinners.forEach { spinner ->
                                val adapter = ArrayAdapter(applicationContext, R.layout.simple_spinner_item, rooms)
                                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                                spinner.adapter = adapter
                                spinner.setSelection(-1)
                            }
